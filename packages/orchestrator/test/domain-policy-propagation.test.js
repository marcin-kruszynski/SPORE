...
import { getExecution, listSteps, openOrchestratorDatabase } from "../src/store/execution-store.js";
...
test("backend domain policy propagates from planner into execution store records", async (t) => {
  ...
  assert.equal(invocation.effectivePolicy.workflowPolicy.stepSoftTimeoutMs, 30000);
  assert.equal(invocation.effectivePolicy.workflowPolicy.stepHardTimeoutMs, 120000);
  assert.equal(invocation.effectivePolicy.workflowPolicy.defaultMaxAttempts, 3);
  assert.equal(invocation.effectivePolicy.workflowPolicy.retryTargetRole, "builder");
  assert.equal(invocation.effectivePolicy.workflowPolicy.resetDescendantSteps, true);
  assert.deepEqual(invocation.effectivePolicy.workflowPolicy.defaultRoles, ["lead", "builder", "tester", "reviewer"]);
  assert.deepEqual(invocation.effectivePolicy.policyPackIds, ["service-core", "platform-backend"]);
  assert.equal(invocation.effectivePolicy.runtimePolicy.sessionModeByRole.builder, "ephemeral");
  assert.equal(invocation.effectivePolicy.runtimePolicy.sessionModeByRole.reviewer, "persistent");
  assert.equal(invocation.effectivePolicy.docsKbPolicy.resultLimit, 7);
  assert.deepEqual(invocation.metadata.policyPacks.map((pack) => pack.id), ["service-core", "platform-backend"]);
