...
import { createExecution, driveExecution } from "../src/execution/workflow-execution.js";
import { listWorkspaceAllocations, openOrchestratorDatabase } from "../src/store/execution-store.js";
...
async function makeTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "spore-builder-tester-"));
...
test("builder handoff snapshot provisions a separate tester verification workspace", async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "spore-builder-tester-flow-"));
  ...
  try {
    ...
    const authoringWorkspace = workspaces.find((workspace) => workspace.stepId === builderStep.id);
    const verificationWorkspace = workspaces.find((workspace) => workspace.stepId === testerStep.id);
    ...
    assert.equal(verificationWorkspace.metadata.workspacePurpose, "verification");
    assert.notEqual(authoringWorkspace.worktreePath, verificationWorkspace.worktreePath);
    assert.equal(verificationWorkspace.metadata.sourceWorkspaceId, authoringWorkspace.id);
    ...
    assert.equal(verificationWorkspace.metadata.sourceCommit, authoringWorkspace.metadata.handoff.snapshotCommit);
    assert.equal(verificationWorkspace.metadata.sourceRef, authoringWorkspace.metadata.handoff.snapshotRef);

    const builderReadme = await fs.readFile(path.join(authoringWorkspace.worktreePath, "README.md"), "utf8");
    const testerReadme = await fs.readFile(path.join(verificationWorkspace.worktreePath, "README.md"), "utf8");
    assert.equal(builderReadme, testerReadme);

    launchContexts.push(path.join(process.cwd(), "tmp", "sessions", `${builderStep.sessionId}.launch-context.json`));
    launchContexts.push(path.join(process.cwd(), "tmp", "sessions", `${testerStep.sessionId}.launch-context.json`));
    ...
    assert.equal(testerLaunch.sourceCommit, authoringWorkspace.metadata.handoff.snapshotCommit);
  } finally {
    ...
    await Promise.all(launchContexts.map((filePath) => fs.rm(filePath, { force: true }).catch(() => {})));
