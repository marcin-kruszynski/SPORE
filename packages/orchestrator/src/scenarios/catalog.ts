// biome-ignore-all lint/suspicious/noExplicitAny: scenario and regression catalog rows are intentionally additive config payloads.
...
async function readConfigDirectory(relativeDir) {
  ...
  for (const filePath of files) {
    const parsed = (await readYaml(filePath)) as Record<string, unknown>;
    items.push({
      ...parsed,
...
export async function getRegressionDefinition(regressionId) {
  ...
  return (
    regressions.find((regression) => regression.id === regressionId) ?? null
  );
}
