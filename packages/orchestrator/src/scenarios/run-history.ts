...
import {
  ...
  normalizeFailureDescriptor,
} from "./failure-descriptor.js";
import { normalizeReportPathMap } from "./report-paths.js";

// biome-ignore lint/suspicious/noExplicitAny: run-history assembles additive operator read models across scenarios, regressions, work items, and self-build state.
...
function classifyScenarioOutcome(
  run: LooseRecord,
  detail: LooseRecord | null = null,
) {
  const finalState = detail?.execution?.state ?? run?.status ?? "unknown";
  const errorMessage =
    run?.metadata?.error ?? run?.assertionSummary?.error ?? null;
  ...
  if (finalState === "failed") {
    return {
      code:
        classifyFailureFromMessage(errorMessage) ??
  ...
  return {
    code:
      classifyFailureFromMessage(errorMessage) ?? "scenario_assertion_failure",
    reason: errorMessage ?? finalState,
  };
}
...
function buildScenarioSuggestedActions(run, detail = null) {
  const executionId =
    run?.metadata?.executionId ?? detail?.execution?.id ?? null;
  const failure = normalizeFailureDescriptor(
    run?.metadata?.failure ?? run?.metadata?.failureClassification,
    {
      reason:
        run?.metadata?.failureReason ?? run?.assertionSummary?.error ?? null,
      source: "scenario-run",
  ...
  if (failure.code === "governance_failure") {
    return [
      buildSuggestion("review-governance", {
        reason:
          "Scenario stopped in a governance state that needs an operator decision.",
        expectedOutcome:
          "The execution moves past review or approval instead of remaining blocked.",
        targetType: "execution",
        targetId: executionId,
        commandHint: executionId
          ? `npm run orchestrator:history -- --execution ${executionId}`
          : null,
        httpHint: executionId
  ...
  if (failure.code === "timeout_or_stall") {
    return [
      buildSuggestion("inspect-execution-history", {
        reason:
          "Scenario stalled or held before it reached a settled success state.",
        expectedOutcome:
          "The operator can see whether the next action is resume, resolve-escalation, or rerun.",
        targetType: "execution",
        targetId: executionId,
        commandHint: executionId
          ? `npm run orchestrator:history -- --execution ${executionId}`
      ...
      buildSuggestion("rerun-scenario", {
        ...
        expectedOutcome:
          "A fresh durable scenario run is created for comparison.",
        ...
        commandHint: run?.id
          ...
          : null,
        httpHint: run?.id
          ? `/scenario-runs/${encodeURIComponent(run.id)}/rerun`
          : null,
        priority: "medium",
      }),
    ];
  ...
  if (failure.infrastructure) {
    return [
      buildSuggestion("inspect-runtime", {
        reason:
          "The failure looks like runtime or launcher infrastructure, not domain logic.",
        expectedOutcome:
          "The operator can isolate the infrastructure fault before re-running the scenario.",
        targetType: "scenario-run",
        targetId: run?.id ?? null,
        commandHint: run?.id
          ...
          : null,
        httpHint: run?.id
          ? `/scenario-runs/${encodeURIComponent(run.id)}/artifacts`
          : null,
        priority: "high",
      }),
      buildSuggestion("rerun-scenario", {
        reason:
          "Re-run the scenario after fixing runtime or gateway setup issues.",
        expectedOutcome:
          "The next run confirms whether the failure was transient infrastructure noise.",
        targetType: "scenario-run",
        targetId: run?.id ?? null,
        commandHint: run?.id
          ? `npm run orchestrator:scenario-rerun -- --run ${run.id}`
  ...
  return [
    buildSuggestion("inspect-execution-history", {
      ...
      expectedOutcome:
        "The operator can inspect the exact execution timeline and policy context before re-running.",
      targetType: "execution",
      targetId: executionId,
      commandHint: executionId
        ? `npm run orchestrator:history -- --execution ${executionId}`
    ...
    buildSuggestion("rerun-scenario", {
      reason:
        "Run the named scenario again after applying a fix or operator decision.",
      expectedOutcome:
        "A new scenario run record is created with comparable artifacts and trend impact.",
      targetType: "scenario-run",
      targetId: run?.id ?? null,
      commandHint: run?.id
        ? `npm run orchestrator:scenario-rerun -- --run ${run.id}`
...
function buildRegressionSuggestedActions(run, items = []) {
  const classifications = sortCountEntries(
    summarizeClassificationCounts(items),
  );
  ...
  if (isInfrastructureFailure(top)) {
    return [
      buildSuggestion("inspect-regression-report", {
        reason:
          "The latest regression failures cluster around infrastructure rather than product logic.",
        expectedOutcome:
          "The operator can identify whether launcher, runtime, or control surfaces regressed.",
        ...
        commandHint: run?.id
          ...
          : null,
        httpHint: run?.id
          ? `/regression-runs/${encodeURIComponent(run.id)}/report`
          : null,
        priority: "high",
      }),
      buildSuggestion("rerun-regression", {
        reason:
          "Retry the regression after fixing the infrastructure issue or confirming it was transient.",
        expectedOutcome:
          "A fresh durable regression run confirms whether the failure persists.",
        targetType: "regression-run",
        targetId: run?.id ?? null,
        commandHint: run?.id
          ...
          : null,
        httpHint: run?.id
          ? `/regression-runs/${encodeURIComponent(run.id)}/rerun`
          : null,
        priority: "medium",
      }),
    ];
  ...
  return [
    buildSuggestion("inspect-regression-report", {
      reason:
        "The regression contains product-facing or governance failures that need triage.",
      expectedOutcome:
        "The operator can see which scenarios failed, why, and which artifacts to open next.",
      targetType: "regression-run",
      targetId: run?.id ?? null,
      commandHint: run?.id
        ? `npm run orchestrator:regression-report -- --run ${run.id}`
    ...
    buildSuggestion("rerun-regression", {
      reason:
        "Re-run the regression after addressing the highest-severity failures.",
      expectedOutcome:
        "The next regression run updates trends and confirms whether the fixes held.",
      targetType: "regression-run",
      targetId: run?.id ?? null,
      commandHint: run?.id
        ? `npm run orchestrator:regression-rerun -- --run ${run.id}`
...
function buildTopFailureSummary(
  items = [],
  fallbackSource = "regression-item",
) {
  const classifications = sortCountEntries(
    summarizeClassificationCounts(items),
  );
  ...
  const sample =
    items.find((item) => item?.metadata?.failureClassification === top.code) ??
    null;
...
function buildFlakySummary(
  runs: LooseRecord[] = [],
  classifier: (run: LooseRecord) => LooseRecord["code"] | boolean = () => false,
) {
  ...
  const alternating = statuses.every(
    (value, index) => index === 0 || value !== statuses[index - 1],
  );
  ...
  if (unstableDurations) {
    return {
      flaky: true,
      flakyReason:
        "Recent run durations vary significantly across the same named flow.",
      confidence: "low",
    };
  }
  return {
...
function _buildTrendHealthBuckets(items = []) {
...
function _decorateRecentScenarioRun(run) {
  const failure = normalizeFailureDescriptor(
    run?.metadata?.failure ?? run?.metadata?.failureClassification,
    {
      reason:
...
function _decorateRecentRegressionRun(run, items = []) {
  return {
    ...run,
    failure: buildTopFailureSummary(items),
    suggestedActions: buildRegressionSuggestedActions(run, items),
    topFailureReasons: sortCountEntries(summarizeClassificationCounts(items)),
    reportPaths: run?.metadata?.reports ?? {},
  };
}

function _toDurationMs(startedAt, endedAt) {
...
function buildRegressionScheduleStatus(
  definition,
  recentRuns = [],
  now = Date.now(),
) {
  ...
  return {
    ...
    reason:
      Number.isFinite(nextDueMs) && now >= nextDueMs
        ? "cadence elapsed"
        : "waiting for next cadence",
    intervalMs,
    lastStartedAt,
    nextDueAt: Number.isFinite(nextDueMs)
...
function summarizeFailureBreakdown(
  items = [],
  getCode = (item) => extractRunFailureCode(item),
) {
  const counts = {};
...
async function writeRegressionReport(run, items) {
  ...
  const topFailureReasons = sortCountEntries(
    summarizeClassificationCounts(items),
  ).map((item) => ({
    ...item,
  ...
  const linkedScenarioRunIds = items
    .map((item) => item.scenarioRunId)
    .filter(Boolean);
  const linkedExecutionIds = items
    .map((item) => item.metadata?.executionId)
    .filter(Boolean);
  const linkedSessionIds = items
    .flatMap((item) => item.metadata?.sessionIds ?? [])
    .filter(Boolean);
...
function normalizeLauncher(flags: LooseRecord = {}) {
  if (flags.stub === true) {
    return "stub";
  }
...
function resolveWorkflowPath(definition) {
  ...
  return workflow.endsWith(".yaml")
    ? workflow
    : `config/workflows/${workflow}.yaml`;
}

function buildScenarioObjective(
...
function buildAssertionSummary(definition, detail) {
  ...
  return {
    ...
    stepCount,
    sessionCount,
    governanceState: ["waiting_review", "waiting_approval"].includes(
      finalState,
    ),
...
function summarizeScenarioRun(definition, run, executionLinks = []) {
  const latestFailure = normalizeFailureDescriptor(
    run?.metadata?.failure ?? run?.metadata?.failureClassification,
    {
      reason:
        run?.metadata?.failureReason ?? run?.assertionSummary?.error ?? null,
  ...
  return {
    ...
    latestExecutionIds: executionLinks
...
function summarizeRegressionRun(definition, run, items = []) {
  ...
  return {
    ...
    latestFailureClassification: latestFailure?.code ?? null,
    latestFailureReason: latestFailure?.reason ?? null,
    latestSuggestedActions: run
      ? buildRegressionSuggestedActions(run, items)
      : [],
...
export async function runScenarioById(
  ...
  try {
    const invocation = await planWorkflowInvocation({
      ...
      maxRoles: Math.max(
        ...
        (definition.roles ?? []).length ||
          Number.parseInt(String(options.maxRoles ?? "1"), 10),
      ),
    ...
    const artifactSummary = await buildScenarioArtifactSummary(
      settledDetail ?? detail,
    );
    ...
    withDatabase(dbPath, (db) => {
      ...
      insertScenarioRunExecution(db, {
        ...
        metadata: {
          ...
          sessionIds: (settledDetail?.sessions ?? []).map(
    ...
    const failedRun = {
      ...
      assertionSummary: {
        ...
        failureClassification:
          classifyFailureFromMessage(error.message) ??
          "scenario_assertion_failure",
      },
      metadata: {
        ...scenarioRun.metadata,
...
export async function listScenarioSummaries(
  ...
  return withDatabase(dbPath, (db) =>
    definitions.map((definition) => {
      ...
      const executionLinks = latestRun
      ...
      const flaky = buildFlakySummary(
        ...
        (run) =>
          run?.metadata?.failureClassification ??
          (isSuccessfulState(run?.status) ? "success" : run?.status),
      );
      return {
        ...summarizeScenarioRun(definition, latestRun, executionLinks),
        latestSuccessfulRun:
          recentRuns.find((run) => isSuccessfulState(run.status)) ?? null,
        latestFailingRun:
          recentRuns.find((run) => !isSuccessfulState(run.status)) ?? null,
...
export async function runRegressionById(
  regressionId,
  options: LooseRecord = {},
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  ...
  for (const scenarioId of definition.scenarios ?? []) {
    ...
    try {
      ...
      const finalStatus = isSuccessfulState(result.run.status)
        ? "passed"
        : "failed";
      ...
      const updatedItem = {
        ...
        metadata: {
          ...
          sessionIds: (result.execution?.sessions ?? [])
            .map((entry) => entry.sessionId)
            .filter(Boolean),
          failureClassification:
            result.run.metadata?.failureClassification ?? null,
...
export async function listRegressionSummaries(
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  ...
  return withDatabase(dbPath, (db) =>
    definitions.map((definition) => {
      ...
      const flaky = buildFlakySummary(
        ...
        (run) =>
          run?.metadata?.failureSummary?.code ??
          (run?.status === "passed" ? "success" : run?.status),
      );
      return {
        ...summarizeRegressionRun(definition, latestRun, items),
        latestSuccessfulRun:
          recentRuns.find((run) => run.status === "passed") ?? null,
        latestFailingRun:
          recentRuns.find((run) => run.status !== "passed") ?? null,
        trendSnapshot: trendPayload.windows.last10,
        trendHealth: classifyTrendHealth(trendPayload.windows.last10),
...
export async function listRegressionHistory(
  regressionId,
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
  limit = 20,
) {
...
export function getScenarioRunDetail(
  runId,
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  return withDatabase(dbPath, (db) => {
...
export async function getScenarioRunSummaryById(
  runId,
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  ...
  const failure = normalizeFailureDescriptor(
    detail.run.metadata?.failure ?? detail.run.metadata?.failureClassification,
    {
      reason:
        detail.run.metadata?.failureReason ??
        detail.run.assertionSummary?.error ??
        null,
...
export async function getScenarioRunArtifacts(
  runId,
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  const detail = getScenarioRunDetail(runId, dbPath);
  ...
  for (const executionLink of detail.executions) {
    const executionDetail = getExecutionDetail(
      executionLink.executionId,
      dbPath,
    );
  ...
  return {
    ...
    failure: normalizeFailureDescriptor(
      detail.run.metadata?.failure ??
        detail.run.metadata?.failureClassification,
      {
        reason:
          detail.run.metadata?.failureReason ??
          detail.run.assertionSummary?.error ??
...
function buildTrendWindow(runs, isSuccess) {
  const normalizedRuns = runs.filter(Boolean).map((run) => ({
    ...run,
    durationMs:
  ...
  return {
    ...
    failCount,
    passRate:
      completedRuns.length > 0 ? passCount / completedRuns.length : null,
    averageDurationMs:
      durations.length > 0
        ? Math.round(
            durations.reduce((sum, value) => sum + value, 0) / durations.length,
...
function buildTrendPayload(
  ...
  kind: string,
  runs: LooseRecord[],
  isSuccess: (run: LooseRecord) => boolean,
  getFailureCode: (
    run: LooseRecord,
  ) => LooseRecord["code"] = extractRunFailureCode,
) {
...
export function getRegressionRunDetail(
  runId,
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  return withDatabase(dbPath, (db) => {
...
export async function getRegressionRunSummaryById(
  runId,
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
...
export async function getRegressionRunReport(
  runId,
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  ...
  const history = await listRegressionHistory(
    detail.run.regressionId,
    dbPath,
    25,
  );
  const reports = normalizeReportPathMap(detail.run?.metadata?.reports);
  const normalized = {};
  ...
  const currentIndex = orderedHistory.findIndex(
    (item) => item.id === detail.run.id,
  );
  const previousRun =
    currentIndex >= 0
      ? (orderedHistory[currentIndex + 1] ?? null)
      : (orderedHistory[1] ?? null);
  const failureClassificationSummary = sortCountEntries(
    summarizeClassificationCounts(detail.items),
  ).map((item) => ({
    ...item,
    label: humanizeClassification(item.code),
    infrastructure: isInfrastructureFailure(item.code),
  ...
  return {
    ...
    linkedScenarioRunIds: detail.items
      .map((item) => item.scenarioRunId)
      .filter(Boolean),
    linkedExecutionIds: detail.items
      .map((item) => item.metadata?.executionId)
      .filter(Boolean),
    linkedSessionIds: detail.items
      .flatMap((item) => item.metadata?.sessionIds ?? [])
      .filter(Boolean),
    ...
    artifactSummary: {
      ...
      linkedScenarioRuns: detail.items
        .map((item) => item.scenarioRunId)
        .filter(Boolean).length,
      linkedExecutions: detail.items
        .map((item) => item.metadata?.executionId)
        .filter(Boolean).length,
      linkedSessions: detail.items
        .flatMap((item) => item.metadata?.sessionIds ?? [])
        .filter(Boolean).length,
    },
...
export async function getRegressionLatestReport(
  regressionId,
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
...
export async function getRegressionScheduleSummary(
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  const definitions = await listRegressionDefinitions();
  return withDatabase(dbPath, (db) =>
...
export async function getRegressionSchedulerStatus(
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
  limit = 10,
) {
  const profiles = await getRegressionScheduleSummary(dbPath);
  const evaluations = withDatabase(dbPath, (db) =>
    listSchedulerEvaluations(db, null, limit),
  );
  return {
    profiles,
...
export async function runRegressionSchedulerOnce(
  options: LooseRecord = {},
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  const schedule = await getRegressionScheduleSummary(dbPath);
  const due = schedule.filter(
    (item) => item.scheduleStatus?.enabled && item.scheduleStatus?.due,
  );
...
export async function runRegressionScheduler(
  options: LooseRecord = {},
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  const definitions = await listRegressionDefinitions();
    ...
    definitions
      ...
      .map((definition) => {
        ...
        return {
          definition,
          recentRuns,
          scheduleStatus: buildRegressionScheduleStatus(
            definition,
            recentRuns,
            now,
          ),
  ...
  const runnable = candidates.filter(
    (entry) => !dueOnly || entry.scheduleStatus.due,
  );
  ...
  withDatabase(dbPath, (db) =>
    updateSchedulerEvaluation(db, settledEvaluation),
  );
  return {
    ...detail,
    evaluation: withDatabase(dbPath, (db) =>
      getSchedulerEvaluation(db, settledEvaluation.id),
    ),
  };
}
...
export async function getRegressionTrends(
  ...
  return {
    ...
    latestFailure: normalizeFailureDescriptor(
      detail.runs.find((run) => run.status !== "passed")?.metadata
        ?.failureSummary,
      {
        reason:
          detail.runs.find((run) => run.status !== "passed")?.metadata
            ?.failureSummary?.reason ?? null,
        source: "regression-trend",
        finalState:
          detail.runs.find((run) => run.status !== "passed")?.status ?? null,
      },
    ),
    links: {
...
export async function rerunScenarioRun(
  runId,
  options: LooseRecord = {},
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
  ...
  return runScenarioById(
    inherited.scenarioId,
    {
      project:
        options.project ??
        inherited.metadata?.projectPath ??
...
export async function rerunRegressionRun(
  runId,
  options: LooseRecord = {},
  dbPath = DEFAULT_ORCHESTRATOR_DB_PATH,
) {
...
export async function getRunCenterSummary(
  ...
  const recent = withDatabase(dbPath, (db) => ({
    ...
    workItemRunsByItem: Object.fromEntries(
      listWorkItems(db, null, limit * 2).map((item) => [
        item.id,
        listWorkItemRuns(db, item.id, 5),
      ]),
    ),
  }));
  const scenarioById = new Map(
    scenarios.map((item) => [item.scenarioId, item]),
  );
  const regressionById = new Map(
    regressions.map((item) => [item.regressionId, item]),
  );
  ...
  const trendBreakdown = {
    stableScenarios: scenarios.filter((item) => item.trendHealth === "stable")
      .length,
    degradingScenarios: scenarios.filter(
      (item) => item.trendHealth === "degrading",
    ).length,
    failingScenarios: scenarios.filter((item) => item.trendHealth === "failing")
      .length,
    stableRegressions: regressions.filter(
      (item) => item.trendHealth === "stable",
    ).length,
    degradingRegressions: regressions.filter(
      (item) => item.trendHealth === "degrading",
    ).length,
    failingRegressions: regressions.filter(
      (item) => item.trendHealth === "failing",
    ).length,
  };
  const flaky = {
    scenarios: scenarios
      .filter(
        (item) =>
          item.trendSnapshot?.flaky ||
          item.trendSnapshot?.flakySummary?.flaky ||
          item.flakySummary?.flaky,
      )
      .map((item) => ({
        id: item.scenarioId,
        label: item.label,
        flakyReason:
          item.flakySummary?.flakyReason ??
          item.trendSnapshot?.flakyReason ??
          null,
      })),
  ...
  const latestReports = regressions
    ...
    .map((item) => ({
      ...
      reportPaths: Object.values(
        item.latestRun?.metadata?.reports ?? {},
      ).filter(Boolean),
      ...
      links: {
        ...
        report: item.latestRunId
          ? `/regression-runs/${encodeURIComponent(item.latestRunId)}/report`
          : null,
        run: item.latestRunId
          ? `/regression-runs/${encodeURIComponent(item.latestRunId)}`
          : null,
      },
    }));
  ...
  const alerts = [
    ...scenarios
      ...
      .map((item) => ({
        ...
        trendHealth:
          item.trendHealth ?? item.trendSnapshot?.health ?? "unknown",
      })),
    ...regressions
      .filter((item) => item.latestFailure?.failed)
      .map((item) => ({
    ...
    .sort((left, right) => {
      ...
      return (
        severityDelta || String(left.label).localeCompare(String(right.label))
      );
    })
    .slice(0, limit);
  ...
  const recommendations = Array.from(
    new Map(
      [
        ...
        `${action.action}:${action.targetType}:${action.targetId}:${action.reason}`,
        action,
      ]),
    ).values(),
  )
  ...
  const recentScenarioRuns = recent.recentScenarioRuns.map((run) => {
    const scenarioSummary = (scenarioById.get(run.scenarioId) ??
    ...
    return {
      ...
      trendHealth:
        scenarioSummary?.trendHealth ??
        scenarioSummary?.trendSnapshot?.health ??
        "unknown",
      failure: normalizeFailureDescriptor(
        run.metadata?.failure ?? run.metadata?.failureClassification,
        {
      ...
      links: {
        ...
        execution: run.metadata?.executionId
          ? `/executions/${encodeURIComponent(run.metadata.executionId)}`
          : null,
      },
    };
  });
  ...
  const recentRegressionRuns = recent.recentRegressionRuns.map((run) => {
    const regressionSummary = (regressionById.get(run.regressionId) ??
      null) as LooseRecord | null;
    return {
      ...run,
      latestRunId: run.id,
      ...
      trendHealth:
        regressionSummary?.trendHealth ??
        regressionSummary?.trendSnapshot?.health ??
        "unknown",
      failure: normalizeFailureDescriptor(
        run.metadata?.failureSummary ?? regressionSummary?.latestFailure,
        {
          reason:
            run.metadata?.failureSummary?.reason ??
            regressionSummary?.latestFailureReason ??
            null,
          source: run.metadata?.failureSummary?.source ?? "regression-run",
          finalState: run.status,
        },
      ...
      suggestedActions:
        run.metadata?.suggestedActions ??
        buildRegressionSuggestedActions(run, []),
  ...
  const recentWorkItemRuns = recent.workItems
    .flatMap((item) =>
      (recent.workItemRunsByItem[item.id] ?? []).map((run) => ({ item, run })),
    )
    .sort((left, right) =>
      String(right.run.startedAt ?? "").localeCompare(
        String(left.run.startedAt ?? ""),
      ),
    )
    .slice(0, limit)
    .map(({ item, run }) => ({
        ...
        run.metadata?.validation?.status === "completed"
          ...
          : [
              buildSuggestion("inspect-work-item", {
                ...
                expectedOutcome:
                  "The operator can review the latest work-item run, proposal, and validation state.",
      ...
      links: {
        ...
        proposal: run.metadata?.proposalArtifactId
          ? `/proposal-artifacts/${encodeURIComponent(run.metadata.proposalArtifactId)}`
          : null,
      },
    }));
  ...
  return {
    counts: {
      ...
      pendingProposalArtifacts: selfBuild.proposals.filter(
        (artifact) => artifact.status !== "approved",
      ).length,
      failingScenarios: scenarios.filter(
        (item) => item.latestRun && !isSuccessfulState(item.latestRun.status),
      ).length,
      failingRegressions: regressions.filter(
        (item) => item.latestRun && item.latestRun.status !== "passed",
      ).length,
