...
// biome-ignore lint/suspicious/noExplicitAny: planner output merges additive project, domain, workflow, and policy payloads with intentionally loose shapes.
...
function resolveDomain(project, domainId) {
  ...
  return (
    project.activeDomains?.find((candidate) => candidate.id === domainId) ??
    null
  );
}
...
async function resolveWorkflowPath(project, domain, inputPath) {
  const candidate =
    normalizeWorkflowInput(inputPath) ??
    pickDefaultWorkflowPath(project, domain);
...
async function resolveDomainConfig(domainId: string | null) {
  if (!domainId) {
  ...
  const result = {
    path: candidate,
    config: (await readYaml(candidate)) as LooseRecord,
  };
  return result;
}
...
async function resolvePolicyPack(packId) {
  ...
  return {
    ...
    path: candidate,
    config: (await readYaml(candidate)) as LooseRecord,
  } satisfies PolicyPackReference;
}

async function resolvePolicyPacks(packIds = []) {
  const packs: PolicyPackReference[] = [];
...
function mergePolicies(
  domainConfig: LooseRecord = {},
  domainOverride: LooseRecord = {},
): PolicyContainer {
  return {
    workflowPolicy: {
      ...mergeObjects(
        domainConfig.workflowPolicy,
        domainOverride.workflowPolicy,
      ),
    ...
    coordinationPolicy: {
      ...mergeObjects(
        domainConfig.coordinationPolicy,
        domainOverride.coordinationPolicy,
      ),
    },
  } satisfies PolicyContainer;
}

function emptyPolicyContainer(): PolicyContainer {
  return {
    workflowPolicy: {},
    runtimePolicy: {},
    docsKbPolicy: {},
...
function mergePolicyChain(items: LooseRecord[] = []): PolicyContainer {
  return items.reduce(
    (accumulator, item) => mergePolicies(accumulator, item),
    emptyPolicyContainer(),
  );
}
...
async function resolveProfilePath(
  ...
  const projectRoleProfileId = !domainId
    ? resolveProjectRoleProfileId(project, role, options.projectRoleProfiles)
    : null;
  const requestedProfileId =
    projectRoleProfileId ??
    (domain && DOMAIN_ROLE_KEYS[role] ? domain[DOMAIN_ROLE_KEYS[role]] : null);
  const effectiveProfileId =
    requestedProfileId ?? DEFAULT_ROLE_PROFILE[role] ?? role;
  const requestedPath = path.join(
    PROJECT_ROOT,
    "config",
    "profiles",
    `${effectiveProfileId}.yaml`,
  );
...
export async function planWorkflowInvocation({
  ...
  const resolvedWorkflowPath = await resolveWorkflowPath(
    project,
    domain,
    workflowPath,
  );
  ...
  const policy = mergePolicyChain([
    ...domainPolicyPacks.map((pack) => (pack.config ?? {}) as LooseRecord),
    (domainConfig.config ?? {}) as LooseRecord,
    domain ?? {},
    policyOverrides ?? {},
  ]);
  ...
  const result = {
    ...
    effectivePolicy: {
      workflowPolicy: {
        ...
        resetDescendantSteps:
          policy.workflowPolicy?.resetDescendantSteps ?? false,
      ...
      coordinationPolicy: {
        autoHoldParentOnOpenChildEscalation:
          policy.coordinationPolicy?.autoHoldParentOnOpenChildEscalation ??
          true,
        ...
        autoHoldParentOnOpenPromotionEscalation:
          policy.coordinationPolicy?.autoHoldParentOnOpenPromotionEscalation ??
          true,
        ...
        escalateOnFamilyStallMs:
          policy.coordinationPolicy?.escalateOnFamilyStallMs ?? null,
      },
...
export async function planProjectCoordination({
  ...
  const selectedDomains =
    asArray(domains).length > 0
      ? asArray(domains)
      : asArray(project.activeDomains).map((domain: LooseRecord) => domain.id);
...
export async function planFeaturePromotion({
  ...
  const effectiveTargetBranch =
    targetBranch ?? promotionPolicy.targetBranch ?? "main";
  const integrationBranchPrefix =
    promotionPolicy.integrationBranchPrefix ?? `spore/${project.id}/promotion`;
    ...
    metadata: {
      ...
      promotion: {
        ...
        allowMechanicalConflictResolution:
          promotionPolicy.allowMechanicalConflictResolution === true,
