...
export async function ensureStoreDirectory(indexPath) {
  await fs.mkdir(path.dirname(indexPath), { recursive: true });
}

export function openDatabase(indexPath) {
...
export function deleteDatabaseFile(indexPath) {
  return fs.rm(indexPath, { force: true });
}

export function deleteDatabaseWalFiles(indexPath) {
  return Promise.all([
...
export function getExistingDocuments(db) {
  const rows = db
    .prepare("SELECT id, path, hash, modified_at AS modifiedAt, chunk_count AS chunkCount FROM documents")
    .all();
...
export function removeDocumentsNotInSet(db, keepPaths) {
  const rows = db.prepare("SELECT path FROM documents").all();
  const removable = rows.map((row) => row.path).filter((current) => !keepPaths.has(current));
...
export function upsertDocuments(db, documents, embeddingProvider) {
...
export function getStatus(db, indexPath) {
...
export function searchChunks(db, queryVector, queryTokens, limit) {
  const rows = db
    .prepare(`
      SELECT
  ...
  return rows
    .map((row) => {
      const tokens = JSON.parse(row.keywordBlob);
      let keywordScore = 0;
      for (const token of queryTokens) {
        keywordScore += tokens.filter((current) => current === token).length * 5;
      ...
      const semanticScore = cosineSimilarity(queryVector, deserializeVector(row.vectorJson));
