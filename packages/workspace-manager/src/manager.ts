...
import type {
  BuildWorkspaceBranchNameOptions,
  BuildWorkspacePathOptions,
  BuildWorkspaceSnapshotRefOptions,
  CommandOptions,
  CommandResult,
  CreateWorkspaceFromSnapshotOptions,
  CreateWorkspaceOptions,
  DeriveWorkspaceDiagnosticsOptions,
  GitWorktreeEntry,
  InspectWorkspaceOptions,
  ListGitWorktreesOptions,
  PublishWorkspaceSnapshotOptions,
  ReconcileWorkspaceOptions,
  ReconcileWorkspaceResult,
  RemoveWorkspaceOptions,
  RemoveWorkspaceResult,
  SnapshotWorkspaceRecord,
  SummarizeWorkspaceChangesOptions,
  WorkspaceChangeEntry,
  WorkspaceChangeStatus,
  WorkspaceChangeSummary,
  WorkspaceDiagnostics,
  WorkspaceInspection,
  WorkspaceNumstatEntry,
  WorkspacePatchArtifact,
  WorkspaceRecord,
  WorkspaceScopeSummary,
  WorkspaceShellSummaryTarget,
  WorkspaceSnapshotPublication,
  WorkspaceStatusEntry,
  WriteWorkspacePatchArtifactOptions,
} from "./types.js";
...
function run(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      ...
      const message =
        stderr || stdout || `${command} failed with code ${exitCode}`;
...
export function buildWorkspaceBranchName({
  projectId = "default",
  ownerType = "work-item-run",
  ownerId,
}: BuildWorkspaceBranchNameOptions): string {
  if (!ownerId) {
  ...
  const normalizedType =
    String(ownerType)
      .trim()
      .replace(/[^a-zA-Z0-9._-]/g, "-") || "work-item-run";
...
}: BuildWorkspacePathOptions): string {
  if (!workspaceId) {
...
export function buildWorkspaceSnapshotRef({
  ...
  stepId,
  attemptCount = 1,
}: BuildWorkspaceSnapshotRefOptions): string {
  if (!stepId) {
...
function parseWorktreeList(output = ""): GitWorktreeEntry[] {
...
}: ListGitWorktreesOptions = {}): Promise<GitWorktreeEntry[]> {
...
export async function inspectWorkspace({
  ...
  worktreePath,
  branchName = null,
}: InspectWorkspaceOptions): Promise<WorkspaceInspection> {
  if (!worktreePath) {
  ...
  const matched =
    worktrees.find((entry) => path.resolve(entry.path) === normalizedPath) ??
    null;
...
export function deriveWorkspaceDiagnostics({
  inspection,
  allocation = null,
}: DeriveWorkspaceDiagnosticsOptions = {}): WorkspaceDiagnostics {
  ...
  return {
    ...
    porcelainCount: Array.isArray(inspection?.porcelain)
      ? inspection.porcelain.length
      : 0,
  };
}
...
export async function createWorkspace({
  ...
  safeMode = true,
  mutationScope = [],
}: CreateWorkspaceOptions): Promise<WorkspaceRecord> {
  if (!workspaceId) {
  ...
  const canonicalRoot = await resolveCanonicalGitRoot(repoRoot);
  const effectiveWorktreeRoot = resolveWorktreeRoot(
    canonicalRoot,
    worktreeRoot,
  );
  const effectiveBranchName =
    branchName ?? buildWorkspaceBranchName({ projectId, ownerType, ownerId });
  const worktreePath = buildWorkspacePath({
    projectId,
    workspaceId,
  ...
  await run(
    "git",
    ["worktree", "add", "-b", effectiveBranchName, worktreePath, baseRef],
    {
      cwd: canonicalRoot,
    },
  );
...
async function getWorkspaceHead({
  worktreePath,
}: {
  worktreePath: string;
}): Promise<string> {
...
export async function publishWorkspaceSnapshot({
...
}: PublishWorkspaceSnapshotOptions): Promise<WorkspaceSnapshotPublication> {
  if (!worktreePath) {
  ...
  await run("git", ["update-ref", snapshotRef, snapshotCommit], {
    cwd: canonicalRoot,
  });
  return {
    repoRoot: canonicalRoot,
...
export async function createWorkspaceFromSnapshot({
  ...
  safeMode = true,
  mutationScope = [],
}: CreateWorkspaceFromSnapshotOptions): Promise<SnapshotWorkspaceRecord> {
  const baseRef = snapshotCommit ?? snapshotRef;
  if (!baseRef) {
    throw new Error(
      "snapshotRef or snapshotCommit is required to create a workspace from snapshot",
    );
  }
...
export async function removeWorkspace({
  ...
  branchName = null,
  keepBranch = false,
}: RemoveWorkspaceOptions): Promise<RemoveWorkspaceResult> {
  if (!worktreePath) {
    throw new Error("worktreePath is required");
  }
  const canonicalRoot = await resolveCanonicalGitRoot(repoRoot);
...
export async function reconcileWorkspace({
  repoRoot = PROJECT_ROOT,
  allocation = null,
}: ReconcileWorkspaceOptions): Promise<ReconcileWorkspaceResult> {
...
function classifyWorkspaceChange(code = ""): WorkspaceChangeStatus {
...
function matchMutationScope(
  filePath: string,
  mutationScope: string[] = [],
): string {
  const normalizedPath = String(filePath ?? "").replace(/\\/g, "/");
  const scopes = Array.isArray(mutationScope)
    ? mutationScope.filter(Boolean)
    : [];
...
function parseWorkspaceStatusLines(output = ""): WorkspaceStatusEntry[] {
  ...
  for (const rawLine of String(output).split(/\r?\n/)) {
    ...
    const filePath = renameParts ? (renameParts.at(-1) ?? rawPath) : rawPath;
    entries.push({
      path: filePath,
      previousPath,
      code,
...
function parseNumstatLines(output = ""): Map<string, WorkspaceNumstatEntry> {
  const byPath = new Map<string, WorkspaceNumstatEntry>();
  for (const rawLine of String(output).split(/\r?\n/)) {
    ...
    byPath.set(filePath, {
      insertions:
        insertionsRaw === "-" ? null : Number.parseInt(insertionsRaw, 10) || 0,
      deletions:
        deletionsRaw === "-" ? null : Number.parseInt(deletionsRaw, 10) || 0,
    });
  }
  return byPath;
}

export async function summarizeWorkspaceChanges({
  worktreePath,
  mutationScope = [],
}: SummarizeWorkspaceChangesOptions): Promise<WorkspaceChangeSummary> {
  if (!worktreePath) {
  ...
  const [statusResult, numstatResult] = await Promise.all([
    ...
    run("git", ["diff", "--numstat", "--find-renames", "HEAD"], {
      cwd: worktreePath,
    }).catch(() => ({
      stdout: "",
      stderr: "",
  ...
  const files: WorkspaceChangeEntry[] = statusEntries.map((entry) => {
    const numstat =
      numstatByPath.get(entry.path) ??
      (entry.previousPath
        ? numstatByPath.get(`${entry.previousPath} => ${entry.path}`)
        : null) ??
      null;
  ...
  const byScope = new Map<string, WorkspaceScopeSummary>();
  const summary = {
    ...
    insertionCount: files.reduce(
      (total, file) => total + (file.insertions ?? 0),
      0,
    ),
    deletionCount: files.reduce(
  ...
  return {
    ...summary,
    changedFiles: files,
    filesByScope: [...byScope.values()].sort((left, right) =>
      left.scope.localeCompare(right.scope),
    ),
  };
}

export async function writeWorkspacePatchArtifact({
  worktreePath,
  outputPath,
}: WriteWorkspacePatchArtifactOptions): Promise<WorkspacePatchArtifact> {
  if (!worktreePath || !outputPath) {
    throw new Error(
      "worktreePath and outputPath are required to write a workspace patch artifact",
    );
  }
...
export function renderWorkspaceShellSummary(
  workspace: WorkspaceShellSummaryTarget,
): string {
  return [
...
export function workspaceCommandHint(
  workspace: WorkspaceShellSummaryTarget,
): string {
  return [
