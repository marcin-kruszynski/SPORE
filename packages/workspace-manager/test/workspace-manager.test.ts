...
async function makeTempRepo() {
  const repoRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "spore-workspace-test-"),
  );
...
test("workspace manager creates, inspects, patches, and removes a git worktree", async () => {
  ...
  assert.equal(
    ...
    buildWorkspacePath({
      projectId: "spore",
      workspaceId,
      worktreeRoot: path.join(repoRoot, ".spore", "worktrees"),
    }),
  );
  ...
  assert.ok(
    worktrees.some(
      (entry) =>
        path.resolve(entry.path) === path.resolve(workspace.worktreePath),
    ),
  );
  ...
  const patch = await writeWorkspacePatchArtifact({
    worktreePath: workspace.worktreePath,
...
test("workspace change summary groups changed files by mutation scope", async () => {
  ...
  await fs.mkdir(path.join(workspace.worktreePath, "docs"), {
    recursive: true,
  });
  await fs.mkdir(path.join(workspace.worktreePath, "apps", "web"), {
    recursive: true,
  });
  await fs.writeFile(
    path.join(workspace.worktreePath, "docs", "guide.md"),
    "# guide\n",
    "utf8",
  );
  await fs.writeFile(
    path.join(workspace.worktreePath, "apps", "web", "panel.js"),
    "console.log('panel');\n",
    "utf8",
  );
  await fs.writeFile(
  ...
  assert.ok(
    summary.filesByScope.some(
      (entry) => entry.scope === "docs" && entry.fileCount === 1,
    ),
  );
  assert.ok(
    summary.filesByScope.some(
      (entry) => entry.scope === "apps/web" && entry.fileCount === 1,
    ),
  );
...
test("workspace manager publishes a builder snapshot and provisions a separate verification workspace from it", async () => {
  ...
  const verificationReadme = await fs.readFile(
  ...
  const verificationReadmeAfterBuilderEdit = await fs.readFile(
    path.join(verificationWorkspace.worktreePath, "README.md"),
    "utf8",
  );
  assert.match(verificationReadmeAfterBuilderEdit, /builder authored change/);
  assert.doesNotMatch(
    verificationReadmeAfterBuilderEdit,
    /builder changed again/,
  );
