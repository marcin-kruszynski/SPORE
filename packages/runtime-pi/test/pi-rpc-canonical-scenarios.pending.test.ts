...
test("stub scenario run exposes durable history, artifacts, and optional next-wave probes", async (t) => {
  ...
  const scenarioSummary = await getJson(
    harness.baseUrl,
    "/scenarios/docs-adr-pass",
  );
  ...
  const scenarioRuns = await getJson(
    harness.baseUrl,
    "/scenarios/docs-adr-pass/runs?limit=5",
  );
...
test("stub regression run exposes durable reports and optional run/trend probes", async (t) => {
  ...
  const regressionSummary = await getJson(
    harness.baseUrl,
  ...
  const regressionRuns = await getJson(
    harness.baseUrl,
    "/regressions/local-fast/runs?limit=5",
  );
