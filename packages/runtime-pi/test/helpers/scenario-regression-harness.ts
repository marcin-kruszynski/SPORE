...
export async function startStubOrchestrator(t, prefix = "scenario-stub") {
  ...
  await fs.mkdir(path.dirname(env.SPORE_ORCHESTRATOR_DB_PATH), {
    recursive: true,
  });
  ...
  const child = spawn(
    process.execPath,
    buildTsxEntrypointArgs("services/orchestrator/server.ts"),
    {
      cwd: PROJECT_ROOT,
      env: {
        ...process.env,
...
export async function probeOptionalJson(
  baseUrl: string,
  routePath: string,
  options: ProbeOptionalJsonOptions = {},
) {
...
export async function runStubRegression(
  baseUrl: string,
  regressionId: string,
