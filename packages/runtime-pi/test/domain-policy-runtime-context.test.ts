...
import { planWorkflowInvocation } from "@spore/orchestrator";
import {
  buildSessionPlan,
  PROJECT_ROOT,
  writeStartupContext,
...
test("planned domain policy becomes runtime retrieval inputs and startup context", async (t) => {
  ...
  assert.deepEqual(
    ...
    launch.policy.docsKbPolicy.queryTerms,
  );
  ...
  assert.equal(
    written.payload.retrieval.results[0].path,
    "docs/test/backend-policy.md",
  );
