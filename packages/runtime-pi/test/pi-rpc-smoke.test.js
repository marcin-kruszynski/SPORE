...
test("pi-rpc smoke run completes and writes runtime artifacts", async (t) => {
  ...
  const transcriptPath = path.join(process.cwd(), "tmp", "sessions", `${sessionId}.transcript.md`);
  const eventsPath = path.join(process.cwd(), "tmp", "sessions", `${sessionId}.pi-events.jsonl`);
  const statusPath = path.join(process.cwd(), "tmp", "sessions", `${sessionId}.rpc-status.json`);
