...
async function startOrchestratorForPi(t, prefix) {
  ...
  await Promise.all([
    fs.mkdir(path.dirname(context.env.SPORE_ORCHESTRATOR_DB_PATH), {
      recursive: true,
    }),
    fs.mkdir(path.dirname(context.env.SPORE_SESSION_DB_PATH), {
      recursive: true,
    }),
    fs.mkdir(path.dirname(context.env.SPORE_EVENT_LOG_PATH), {
      recursive: true,
    }),
  ]);
...
async function runCanonicalScenario(t, scenarioId) {
  ...
  const response = await postJson(
    `${harness.baseUrl}/scenarios/${scenarioId}/run`,
    {
      launcher: "pi-rpc",
      wait: true,
      by: "pi-e2e",
  ...
  assert.ok(
    ["completed", "waiting_review", "waiting_approval", "held"].includes(
      response.json?.run?.status,
    ),
  );
  ...
  assert.ok(
    runs.json?.detail?.runs?.some((item) => item.id === response.json.run.id),
  );
