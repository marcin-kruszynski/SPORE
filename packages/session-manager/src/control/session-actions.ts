...
import type {
  AppendSessionEventRecordOptions,
  SessionEvent,
  SessionRecord,
  SessionRecordOverrides,
  TransitionSessionStateOptions,
} from "../types.js";
...
export async function appendSessionEventRecord({
  ...
  type,
  payload = {},
}: AppendSessionEventRecordOptions): Promise<{
  session: SessionRecord;
  event: SessionEvent;
}> {
  const resolvedDbPath = resolvePath(dbPath, DEFAULT_SESSION_DB_PATH);
  const resolvedEventLogPath = resolvePath(
    eventLogPath,
    DEFAULT_EVENT_LOG_PATH,
  );
...
export async function transitionSessionState({
  ...
  overrides = {},
  payload = {},
}: TransitionSessionStateOptions): Promise<{
  ...
  try {
    ...
    const updated = transitionSessionRecord(
      applyOverrides(current, overrides),
      nextState,
    );
