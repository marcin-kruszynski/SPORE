...
test("gateway steer and stop scaffolding works with a real PI session", async (t) => {
  ...
  const initialLive = await getLiveSession(
    harness.baseUrl,
    launched.sessionId,
    { limit: 20 },
  );
  ...
  const steer = await postJson(
    ...
    {
      message:
        "Please acknowledge this steering message with the token SPORE_STEER_OK.",
      mode: "follow_up",
  ...
  const steerHistory = await getControlHistory(
    harness.baseUrl,
    launched.sessionId,
    { limit: 10 },
  );
  ...
  assert.ok(
    steerHistory.json.controlHistory.some(
      (entry) => entry.id === steer.json.request.id,
    ),
  );

  const steerStatus = await getControlStatus(
    harness.baseUrl,
    launched.sessionId,
    steer.json.request.id,
  );
  ...
  const liveSnapshotAfterSteer = await getLiveSession(
    harness.baseUrl,
    launched.sessionId,
    { limit: 30 },
  );
  ...
  const liveStopped = await getLiveSession(
    harness.baseUrl,
    launched.sessionId,
    { limit: 30 },
  );
  ...
  const summary = await readGatewayArtifactSummary(
    harness.baseUrl,
    launched.sessionId,
  );
  ...
  const controlArtifact = await readGatewayArtifacts(
    harness.baseUrl,
    launched.sessionId,
    "control",
  );
  ...
  const liveStoppedDiagnostics = await getLiveSession(
    harness.baseUrl,
    launched.sessionId,
    { limit: 30 },
  );
...
test("gateway mark-complete scaffolding works with a real PI session", async (t) => {
  ...
  if (active.session.state !== "active") {
    t.skip(
      `session settled before mark-complete window (${active.session.state})`,
    );
    return;
  }
  ...
  const exitArtifact = await readGatewayArtifacts(
    harness.baseUrl,
    launched.sessionId,
    "exit",
  );
  ...
  const liveCompletedDiagnostics = await getLiveSession(
    harness.baseUrl,
    launched.sessionId,
    { limit: 30 },
  );
