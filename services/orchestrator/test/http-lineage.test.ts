...
import { createExecution, planWorkflowInvocation } from "@spore/orchestrator";
...
test("orchestrator tree and branch APIs expose lineage-aware execution graphs", async (t) => {
  ...
  const spawnResponse = await fetch(
    `http://127.0.0.1:${ORCHESTRATOR_PORT}/executions/${executionId}/branches`,
    {
      method: "POST",
      headers: {
  ...
  const orchestratorTreeResponse = await fetch(
    `http://127.0.0.1:${ORCHESTRATOR_PORT}/executions/${executionId}/tree`,
  );
  ...
  assert.equal(
    orchestratorTree.tree.root.children[0].execution.parentExecutionId,
    executionId,
  );
  assert.equal(
    orchestratorTree.tree.root.children[1].execution.parentExecutionId,
    executionId,
  );
  assert.ok(orchestratorTree.tree.root.stepSummary.count >= 3);

  const webTreeResponse = await fetch(
    `http://127.0.0.1:${WEB_PORT}/api/orchestrator/executions/${executionId}/tree`,
  );
  ...
  const holdResponse = await fetch(
    `http://127.0.0.1:${ORCHESTRATOR_PORT}/executions/${executionId}/tree/hold`,
    {
      method: "POST",
      headers: {
        "content-type": "application/json",
  ...
  assert.deepEqual(
    heldTree.changedExecutionIds.sort(),
    [
      executionId,
