...
const GATEWAY_ORIGIN = process.env.SPORE_GATEWAY_ORIGIN ?? "http://127.0.0.1:8787";
const ORCHESTRATOR_ORIGIN = process.env.SPORE_ORCHESTRATOR_ORIGIN ?? "http://127.0.0.1:8789";
...
async function proxyRequest(request, response) {
  const gatewayTarget = request.url.startsWith("/api/orchestrator/")
    ? new URL(request.url.replace(/^\/api\/orchestrator/, ""), ORCHESTRATOR_ORIGIN)
