...
test("self-build dashboard exposes dedicated operator-first surface with overview, urgent queue, and drilldown navigation", async (t) => {
  ...
  const { dbPath, sessionDbPath, eventLogPath } = await makeTempPaths("spore-web-self-build-dashboard-");
  ...
  assert.ok(html.includes("SPORE") || html.includes("Operator"), "HTML should contain operator page content");
  assert.ok(html.includes("app.js"), "HTML should load app.js");
  ...
  const summaryViaWeb = await getJson(`http://127.0.0.1:${WEB_PORT}/api/orchestrator/self-build/summary`);
  ...
  assert.ok(typeof summaryViaWeb.json.detail.overview.urgentCount === "number", "Overview should have urgentCount");
  assert.ok(typeof summaryViaWeb.json.detail.overview.followUpCount === "number", "Overview should have followUpCount");
  
  // Verify urgent and follow-up queues exist
  assert.ok(Array.isArray(summaryViaWeb.json.detail.urgentWork), "Urgent work queue should be an array");
  assert.ok(Array.isArray(summaryViaWeb.json.detail.followUpWork), "Follow-up work queue should be an array");
  
  // Verify display metadata and freshness cues
  assert.ok(summaryViaWeb.json.detail.displayMetadata, "Display metadata should exist");
  assert.ok(summaryViaWeb.json.detail.freshness, "Freshness cues should exist");
  assert.ok(summaryViaWeb.json.detail.freshness.lastRefresh, "Last refresh timestamp should exist");
  ...
  const workItemsViaWeb = await getJson(`http://127.0.0.1:${WEB_PORT}/api/orchestrator/work-items`);
  ...
  const foundItem = workItemsViaWeb.json.detail.find((item) => item.id === itemId);
  ...
  assert.ok(foundItem.links.self, "Work item should have self link for detail drilldown");

  // Test 5: Verify work-item-groups list is accessible through web proxy
  const groupsViaWeb = await getJson(`http://127.0.0.1:${WEB_PORT}/api/orchestrator/work-item-groups`);
  assert.equal(groupsViaWeb.status, 200);
  assert.ok(groupsViaWeb.json.ok);
  assert.ok(Array.isArray(groupsViaWeb.json.detail));
  
  const foundGroup = groupsViaWeb.json.detail.find((group) => group.id === groupId);
  ...
  const itemDetailViaWeb = await getJson(`http://127.0.0.1:${WEB_PORT}/api/orchestrator/work-items/${encodeURIComponent(itemId)}`);
  ...
  assert.ok(itemDetailViaWeb.json.detail.workItemGroup, "Work item detail should include group linkage");
  assert.equal(itemDetailViaWeb.json.detail.workItemGroup.id, groupId);
  assert.ok(itemDetailViaWeb.json.detail.goalPlan, "Work item detail should include goal plan linkage");
  assert.ok(itemDetailViaWeb.json.detail.links, "Work item detail should include navigation links");

  // Test 7: Verify work-item-group detail is accessible through web proxy
  const groupDetailViaWeb = await getJson(`http://127.0.0.1:${WEB_PORT}/api/orchestrator/work-item-groups/${encodeURIComponent(groupId)}`);
  ...
  assert.ok(Array.isArray(groupDetailViaWeb.json.detail.items), "Group detail should list items");

  // Test 8: Refresh self-build summary and verify updated state
  const refreshedSummary = await getJson(`http://127.0.0.1:${WEB_PORT}/api/orchestrator/self-build/summary`);
  ...
  assert.ok(refreshedSummary.json.detail.counts.workItems >= 1, "Summary should reflect at least one work item");
  assert.ok(refreshedSummary.json.detail.counts.groups >= 1, "Summary should reflect at least one group");
});
